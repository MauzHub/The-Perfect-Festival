...
const FORMATIONS = {
  // BILLINGS — poster layout. Slots are real festival stages (not tier labels), laid out in clean
  // rows top->bottom; tier drives size on the poster (1 = headliners, biggest). Every billing spans
  // all five genres (even range bonus); only the lean differs. Family counts are balance-calibrated.
  'balanced': { name:'Balanced Weekend', slots:[
    {id:'HF', label:'Friday',     fam:'POP',  tier:1, x:25, y:10},
    {id:'HS', label:'Saturday',   fam:'ROCK', tier:1, x:50, y:8},
    {id:'HU', label:'Sunday',     fam:'HIP',  tier:1, x:75, y:10},
    {id:'MA', label:'Sunset',     fam:'RNB',  tier:2, x:34, y:30},
    {id:'MB', label:'Main Stage', fam:'POP',  tier:2, x:66, y:30},
    {id:'SA', label:'River Stage',fam:'ROCK', tier:3, x:30, y:48},
    {id:'SB', label:'Dance Stage',fam:'ELEC', tier:3, x:70, y:48},
    {id:'TA', label:'The Tent',   fam:'ELEC', tier:4, x:30, y:65},
    {id:'TB', label:'Soul Tent',  fam:'RNB',  tier:4, x:70, y:65},
    {id:'PA', label:'Park Stage', fam:'POP',  tier:5, x:35, y:83},
    {id:'PB', label:'Late Night', fam:'HIP',  tier:5, x:65, y:83},
  ]},
  'dance': { name:'Dance-Forward', slots:[
    {id:'HF', label:'Friday',     fam:'ELEC', tier:1, x:25, y:10},
    {id:'HS', label:'Saturday',   fam:'ELEC', tier:1, x:50, y:8},
    {id:'HU', label:'Sunday',     fam:'POP',  tier:1, x:75, y:10},
    {id:'MA', label:'Main Stage', fam:'POP',  tier:2, x:34, y:30},
    {id:'MB', label:'Sunset',     fam:'RNB',  tier:2, x:66, y:30},
    {id:'SA', label:'Dance Stage',fam:'ELEC', tier:3, x:30, y:48},
    {id:'SB', label:'Park Stage', fam:'POP',  tier:3, x:70, y:48},
    {id:'TA', label:'Warehouse',  fam:'ELEC', tier:4, x:30, y:65},
    {id:'TB', label:'Rap Tent',   fam:'HIP',  tier:4, x:70, y:65},
    {id:'PA', label:'River Stage',fam:'ROCK', tier:5, x:35, y:83},
    {id:'PB', label:'Late Night', fam:'HIP',  tier:5, x:65, y:83},
  ]},
  'rock': { name:'Rock-Heavy', slots:[
    {id:'HF', label:'Friday',     fam:'ROCK', tier:1, x:25, y:10},
    {id:'HS', label:'Saturday',   fam:'ROCK', tier:1, x:50, y:8},
    {id:'HU', label:'Sunday',     fam:'POP',  tier:1, x:75, y:10},
    {id:'MA', label:'Main Stage', fam:'ROCK', tier:2, x:34, y:30},
    {id:'MB', label:'Sunset',     fam:'RNB',  tier:2, x:66, y:30},
    {id:'SA', label:'River Stage',fam:'ROCK', tier:3, x:30, y:48},
    {id:'SB', label:'Park Stage', fam:'POP',  tier:3, x:70, y:48},
    {id:'TA', label:'Dance Tent', fam:'ELEC', tier:4, x:30, y:65},
    {id:'TB', label:'Rap Tent',   fam:'HIP',  tier:4, x:70, y:65},
    {id:'PA', label:'Dance Stage',fam:'ELEC', tier:5, x:35, y:83},
    {id:'PB', label:'Late Night', fam:'HIP',  tier:5, x:65, y:83},
  ]},
};
...
function isActPicked(p){ for (const k in state.picks){ if (state.picks[k].player.i === p.i) return true; } return false; }
// True if this source still holds an un-booked act that fits one of the OPEN stages.
function comboHasEligible(c){
  const squad = DATA.squads[`${c[0]}|${c[1]}`] || [];
  const openFams = new Set(openSlots().map(s => s.fam));
  if (!openFams.size) return true;
  return squad.some(p => !isActPicked(p) && [...eligFamilies(p)].some(f => openFams.has(f)));
}
function pickCombo(){
...
const GOAT_T = 95.20, INV_T = 94.25, CENT_T = 93.45;   // sold-out tour / sell-out tour / headline-act floors
const B_CHAMP = 91.40, B_CL = 89.40, B_EUR = 87.40, B_MID = 84.40;   // headliner / main-stage / big-top / mid-bill bands
...
const RECORD_ANCHORS = [
  ...
  [68.15,   1,  3],   // cancelled-tier: a near-empty run
  [72.15,   3,  5],
  [76.40,   4,  8],
  [80.40,   8,  9],
  [84.40,  12, 11],   // mid-bill floor
  [86.40,  16, 10],
  [87.40,  18, 10],   // big-top floor
  [88.40,  20,  9],
  [89.40,  22,  9],   // main-stage floor
  [90.40,  25,  8],
  [91.40,  27,  8],   // headliners floor
